...
def aggregate_setlist_features(df, target_showdate):
    ...
    if isinstance(target_showdate, str):
        try:
            ...
            try:
                ...
                raise ValueError(f"target_showdate '{target_showdate}' does not match '%Y-%m-%d' or '%m-%d-%Y'")
    ...
    showdata = df[['showid', 'showdate']].drop_duplicates().sort_values('showdate', ascending=False)
    ...
    for song, row in song_groups.iterrows():
        ...
        play_show_indices = [i for i, sid in enumerate(window_showids)
                             if sid in unique_showids]
