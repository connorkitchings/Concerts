...
def aggregate_setlist_features(df, target_show_date):
    ...
    showdata = df[['show_id', 'show_date']].drop_duplicates().sort_values('show_date', ascending=False)
    ...
    for song, row in song_groups.iterrows():
        ...
        play_show_indices = [i for i, sid in enumerate(window_show_ids)
                             if sid in unique_show_ids]
    ...
    agg_df = agg_df[~agg_df['last_time_played_date'].isin(last_3_shows)].sort_values("times_played_last_year", ascending=False)
    return agg_df
