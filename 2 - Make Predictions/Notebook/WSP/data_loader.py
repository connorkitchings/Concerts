...
def load_setlist_and_showdata(setlist_path, showdata_path, songdata_path):
    ...
    show_df = pd.read_csv(showdata_path, usecols=["date", "venue", "city", "state", "link"])
    ...
    logger.info(f"Loaded setlist ({len(df):,}) rows from {setlist_path}, shows from {showdata_path}, songs from {songdata_path}")
    return df
