...
data_folder = os.path.abspath(os.path.join(SCRIPT_DIR, "../../../3 - Data/UM/"))
setlist_path = os.path.join(data_folder, "AllThingsUM/setlistdata.csv")
venuedata_path = os.path.join(data_folder, "AllThingsUM/venuedata.csv")
songdata_path = os.path.join(data_folder, "AllThingsUM/songdata.csv")
...
def evaluate_historical_accuracy(num_shows=50):
    ...
    for top_n in [25, 50]:
        ...
        for idx, row in target_shows.iterrows():
            ...
            log_info(f"Show {showdate.date()} (top_{top_n}): {sum(1 for song in actual_songs if song in predicted_songs)}/{len(actual_songs)} correct ({accuracy:.2%})")
