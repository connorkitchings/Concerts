...
def format_dataframe(df: pd.DataFrame, expected_cols: List[str], display_cols: List[str]) -> pd.DataFrame:
    """
...
def get_column_config(file_label: str, band: str) -> Tuple[str, List[str], List[str]]:
    ...
    if file_label == "CK+":
        ...
        if band == "WSP":
            ...
            display_columns = ["Song", "Times Played Last Year", "LTP Date", "Average Gap", "Median Gap"]
        else:
