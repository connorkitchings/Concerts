...
def get_setlist_from_link(link):
    """
    Scrape setlist data from a single link.
    ...
    try:
        ...
        logger.exception("Error fetching/parsing setlist from %s: %s", link, e)
        return pd.DataFrame(columns=EXPECTED_COLUMNS)
    ...
    setlist_raw["set"] = setlist_raw["Raw"].apply(
        lambda x: x[:3]
        if x[:2] in (["0:", "1:", "2:", "3:", "4:", "E:", "E1", "E2", "E3", "E4"])
        else "Notes"
    )
    setlist_raw["set"] = setlist_raw["set"].apply(
        lambda x: "Details" if x[:2] == "??" or re.match(r"^\d{2}/", x[:2]) else x
    )
    setlist_raw["set"] = setlist_raw["set"].str.strip()
    setlist_raw["Raw"] = setlist_raw["Raw"].str.lstrip()
    setlist_raw["Raw"] = np.where(
        setlist_raw["set"].isin(["0", "1", "2", "3", "4", "E"]),
    ...
    songs["song_name"] = (
        songs["song_name"].str.upper().str.replace("*", "", regex=False)
    )
    conditions = [
        songs["song_name"].isin(["???", "ARU/WSP JAM"]),
        songs["song_name"] == "THIS MUST BE THE PLACE (NA<EF>VE MELODY)",
        songs["song_name"] == "W<CR>M",
    ]
    ...
    if is_notes > 0:
        try:
            notes_df = setlist_raw.loc[setlist_raw["set"].isin(["Notes"])].loc[
                :, ["Raw"]
            ]
            if len(notes_df) < 2:
                logger.warning(
                    "Notes DataFrame too short for link %s. Skipping notes parsing.",
                    link,
                )
                ...
                notes_split = [
                    s for s in notes_split if s.startswith(" ") or s.startswith("*")
                ]
                notes_df = pd.DataFrame(
                    {
                        "song_notes_key": notes_split[::2],
                        "song_note_detail": notes_split[1::2],
                    }
                )
                if link == "http://everydaycompanion.com/setlists/20091017a.asp":
                    notes_df = pd.DataFrame(
                        {
                            "song_notes_key": ["*"],
                            "song_note_detail": ["with The Allman Brothers"],
                        }
                    )
                if link == "http://everydaycompanion.com/setlists/20161030a.asp":
                    notes_df = pd.DataFrame(
                        {
                            "song_notes_key": ["*"],
                            "song_note_detail": ["Steve Lopez on Percussion"],
                        }
                    )
                    notes_df["song_note_detail"] = notes_df[
                        "song_note_detail"
                    ].str.lstrip()
                songs_final = pd.merge(
                    songs,
                    notes_df,
                    how="left",
                    left_on="song_notes_key",
                    right_on="song_notes_key",
                )
                songs_final.drop(columns=["song_notes_key", "notes_id"], inplace=True)
        except Exception as e:
            logger.exception("Error parsing notes for link %s: %s", link, e)
            songs_final = songs.copy()
            songs_final["song_note_detail"] = ""
    else:
        songs_final = songs.copy()
...
def load_setlist_data(
    link_list: list, method: str = "all", existing_setlist_data: "pd.DataFrame" = None
) -> "pd.DataFrame":
    """
    ...
    Args:
        ...
        existing_setlist_data (pd.DataFrame, optional):
            Existing setlist data to update. Defaults to None.
    ...
    if existing_setlist_data is not None:
        logger.info(
            "Loaded existing setlistdata.csv with %d rows.", len(existing_setlist_data)
        )
    else:
    ...
    if method == "all":
        ...
        for link in link_list:
            ...
            if setlist is not None and not setlist.empty:
                ...
                if match:
                    ...
                    try:
                        formatted_date = datetime.strptime(showdate, "%Y%m%d").strftime(
                            "%m/%d/%Y"
                        )
                    except ValueError:
                        formatted_date = showdate
                    logger.warning(
                        "No setlist data returned for date %s, skipping.",
                        formatted_date,
                    )
                else:
                    logger.warning(
                        "No setlist data returned for link %s, skipping.", link
                    )
        ...
        filtered_link_list = [
            link
            for link in link_list
            if str(datetime.today().year) in link.split("/")[-1]
            or str(datetime.today().year - 1) in link.split("/")[-1]
        ]
        ...
        for link in filtered_link_list:
            ...
            if setlist is not None and not setlist.empty:
                ...
                all_setsets = all_setsets.drop_duplicates(
                    subset=[
                        "link",
                        "song_name",
                        "set",
        ...
        logger.error("Unknown method '%s' for load_setlist_data.", method)
        return pd.DataFrame(columns=EXPECTED_COLUMNS)
