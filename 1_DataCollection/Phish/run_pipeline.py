...
from Phish.config import (  # Import band-specific LOG_FILE_PATH
    API_KEY,  # Import API_KEY directly from config
    DATA_COLLECTED_DIR,
    LOG_FILE_PATH,
)
...
def main() -> None:
    ...
    if prev_update:
        try:
            ...
            logger.warning(
                f"Could not parse previous update timestamp: {prev_update}. Logging as is."
            )
    ...
    try:
        ...
        logger.info(
            f"load_setlist_data returned. Setlist data rows: {len(setlist_data):,}, Transition data rows: {len(transition_data):,}"
        )
        save_phish_data(
