        ...
        try:
            ...
            tour_data["link"] = [
                ...
                + (
                    ...
                    if parse_date(d) is not pd.NaT
                    else "NA"
                )
                + "a.asp"
                for d in tour_data["date"]
            ]
            ...
            tour_data["date"] = pd.to_datetime(
                tour_data[["year", "month", "day"]], errors="coerce"
            )
            # Venue parsing
            venue_split = tour_data["venue"].str.split(", ")
            tour_data["state"] = venue_split.apply(
                lambda x: x[-1] if len(x) > 0 else None
            )
            tour_data["city"] = venue_split.apply(
                lambda x: x[-2] if len(x) > 1 else None
            )
            tour_data["venue_name"] = venue_split.apply(
                lambda x: ", ".join(x[:-2])
                if len(x) > 2
...
def process_setlist(setlist_link):
    ...
    try:
        ...
        # Songs extraction
        songs = setlist_raw[
            ~setlist_raw["set"].isin(["Details", "Song_Notes", "Show_Notes", "Other"])
        ].copy()
        # Split by ',' and ' > '
        ...
        songs["song_notes_key"] = np.where(
            songs["notes_id"] == 0, "", "*" * songs["notes_id"]
        )
        ...
        raw_notes = setlist_raw[
            setlist_raw["set"].isin(["Song_Notes", "Show_Notes", "Other"])
        ].copy()
        # Show notes
        ...
        if "Show_Notes" in raw_notes["set"].values:
            ...
            show_notes_df = pd.DataFrame(
                {"link": [setlist_link], "show_notes": [" ".join(show_notes)]}
            )
        # Song notes
        notes_df = pd.DataFrame(
            {
                "link": [setlist_link],
                "song_notes_key": [np.nan],
                "song_note_detail": [""],
            }
        )
        if "Song_Notes" in raw_notes["set"].values:
            notes_str = " ".join(
                raw_notes[raw_notes["set"] == "Song_Notes"]["Raw"].tolist()
            )
            notessplit = re.split(r"(?<=[A-Za-z])\*", notes_str)
            notessplit = (
                [notessplit[0]] + ["*" + n for n in notessplit[1:]]
                if len(notessplit) > 1
                else notessplit
            )
            notes_df = pd.DataFrame(
...
def load_all_data(start=1986, end=2025):
    ...
    print(
        f"{len(show_dim)} Historical & {len(fut_dim)} Future Shows Loaded - Now Loading Setlists"
    )
    # Load setlists for historical shows
    songs = pd.concat(
        [process_setlist(link) for link in show_dim["link"]], ignore_index=True
    )
    # Post-process songs
    ...
    songs = (
        songs.groupby("link")
        .apply(lambda df: df.assign(min_set=df["set"].min(), max_set=df["set"].max()))
        .reset_index(drop=True)
    )
    songs["set"] = np.where(
        (songs["set"] == 0)
        & (songs["min_set"] == 0)
        & (songs["max_set"].isin([99, 0])),
        1,
        songs["set"],
    )
    ...
    # Song fact table
    dim_songs = (
        songs.groupby(["link", "show_notes"])
        .agg(n_songs=("song_index", "max"))
        .reset_index()
    )
    ...
    dim["is_radio"] = (
        dim["venue_full"]
        .str.contains(
            r"\b\d+\.\d+FM\b|\b\d+\.\d\b|NBC STUDIOS|ED SULLIVAN THEATER|STUDIO 6B, ROCKAFELLER CENTER|CNN STUDIOS| STUDIO| RECORD",
            regex=True,
        )
        .astype(int)
    )
    dim["is_soundcheck"] = (
        dim["show_notes"]
        .fillna("")
        .str.contains(r"\[Soundcheck; ", regex=True)
        .astype(int)
    )
    dim["is_opening_act"] = (
        dim["show_notes"].fillna("").str.lower().str.contains("opened for").astype(int)
    )
    ...
    print(
        f"Successfully Loaded {dim['link'].nunique()} Widespread Panic Shows ({len(songs)} Total Songs) from {dim['year'].min()} to {dim['year'].max()}"
    )
