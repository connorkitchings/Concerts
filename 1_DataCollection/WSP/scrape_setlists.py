...
def get_setlist_from_link(link):
    ...
    if setlist_raw.empty or 0 not in setlist_raw.columns:
        logger.warning(
            f"Setlist table at {link} is empty or missing expected columns. Skipping."
        )
        return pd.DataFrame(columns=EXPECTED_COLUMNS)
    ...
    if is_notes > 0:
        try:
            ...
            if len(notes_df) < 2:
                logger.warning(
                    f"Notes DataFrame too short for link {link}. Skipping notes parsing."
                )
...
def load_setlist_data(
    ...
    if existing_setlist_data is not None:
        logger.info(
            f"Loaded existing setlistdata.csv with {len(existing_setlist_data):,} rows."
        )
    else:
        ...
        for link in link_list:
            ...
            if setlist is not None and not setlist.empty:
                ...
                if match:
                    ...
                    logger.warning(
                        f"No setlist data returned for date {formatted_date}, skipping."
                    )
                else:
                    logger.warning(
                        f"No setlist data returned for link {link}, skipping."
                    )
