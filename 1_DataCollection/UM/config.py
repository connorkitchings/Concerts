...
DATA_COLLECTED_DIR: Path = Path(os.environ.get("UM_DATA_DIR", str(common_config.PROJECT_ROOT / '3_DataStorage' / BAND_NAME / 'Collected')))
DATA_META_DIR: Path = Path(os.environ.get("UM_DATA_DIR", str(common_config.PROJECT_ROOT / '3_DataStorage' / BAND_NAME / 'Meta')))
...
SETLIST_DATA_FILENAME: str = os.environ.get('UM_SETLIST_DATA_FILENAME', 'setlistdata.csv')
NEXT_SHOW_FILENAME: str = os.environ.get('UM_NEXT_SHOW_FILENAME', 'next_show.json')
LAST_UPDATED_FILENAME: str = os.environ.get('UM_LAST_UPDATED_FILENAME', 'last_updated.json')
