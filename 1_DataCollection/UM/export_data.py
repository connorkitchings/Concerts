from UM.config import (DATA_COLLECTED_DIR, SONG_DATA_FILENAME, VENUE_DATA_FILENAME, 
                     SETLIST_DATA_FILENAME, NEXT_SHOW_FILENAME, LAST_UPDATED_FILENAME, 
                     LOG_FILE_PATH)
SHOW_DATA_FILENAME = 'showdata.csv'  # New output file
from datetime import date # datetime is imported via UM.utils
import pandas as pd
import json
from pathlib import Path
from logger import get_logger
from UM.utils import get_date_and_time # For consistent timestamping

logger = get_logger(__name__, log_file=LOG_FILE_PATH, add_console_handler=True)


def save_um_data(song_data: pd.DataFrame, venue_data: pd.DataFrame, setlist_data: pd.DataFrame, data_dir: Path | None = None) -> None:
