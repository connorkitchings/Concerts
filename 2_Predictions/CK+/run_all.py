...
def run_band(band: str, scripts: dict) -> int:
    ...
    if result_pred.returncode == 0:
        ...
        logger.error(
            f"Predictions for {band} failed with exit code {result_pred.returncode}."
        )
    ...
    result_hist = subprocess.run(
        [sys.executable, hist_path], capture_output=True, text=True
    )
    ...
    if result_hist.returncode == 0:
        ...
        logger.error(
            f"Accuracy update for {band} failed with exit code {result_hist.returncode}."
        )
...
def main() -> None:
    ...
    logger.info(
        f"All band prediction and accuracy pipelines completed in {total_duration:.2f} seconds."
    )
