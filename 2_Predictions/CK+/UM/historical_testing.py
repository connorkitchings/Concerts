...
def evaluate_historical_accuracy(num_shows: int = 50) -> None:
    ...
    data_folder = SCRIPT_DIR.parent.parent.parent / "3 - Data/UM/AllThingsUM"
    setlist_path = data_folder / "setlistdata.csv"
    showdata_path = data_folder / "venuedata.csv"
    songdata_path = data_folder / "songdata.csv"
    predictions_dir = SCRIPT_DIR.parent.parent.parent / "3 - Data/UM/Predictions/ckplus"
    ...
    show_order = df[['show_date', 'venue']].drop_duplicates().sort_values('show_date').reset_index(drop=True)
    show_order['show_num'] = show_order.index + 1
    df = df.merge(show_order[['show_date', 'venue', 'show_num']], on=['show_date', 'venue'], how='left')
    ...
    for top_n in [25, 50]:
        ...
        results[top_n] = sorted(results[top_n], key=lambda x: x['show_date'], reverse=True)
    # Save results
    combined_results = {
        'top_25': results[25],
        'top_50': results[50]
    }
    save_json_path = predictions_dir / "ckplus_accuracy.json"
