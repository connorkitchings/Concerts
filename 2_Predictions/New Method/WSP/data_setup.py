...
def setup_data(test_date):
    ...
    df_all["is_last_6_months"] = np.where(
        (test_date - df_all["date"]) <= pd.DateOffset(days=365 / 2), 1, 0
    )
    df_all["is_last_year"] = np.where(
        (test_date - df_all["date"]) <= pd.DateOffset(days=365), 1, 0
    )
    df_all["is_last_2_years"] = np.where(
        (test_date - df_all["date"]) <= pd.DateOffset(days=365 * 2), 1, 0
    )
    df_all["is_last_4_years"] = np.where(
        (test_date - df_all["date"]) <= pd.DateOffset(days=365 * 4), 1, 0
    )
    df_all["is_last_10_years"] = np.where(
        (test_date - df_all["date"]) <= pd.DateOffset(days=365 * 10), 1, 0
    )
    ...
    df_all = df_all.sort_values(
        ["song_name", "show_index_overall", "date"]
    ).reset_index(drop=True)

    # Create running count for songs
    df_all["running_count"] = df_all.groupby("song_name")["song_name"].transform(
        "cumcount"
    )
    df_all["inverted_running_count"] = (
        df_all["running_count"].max() - df_all["running_count"] + 1
    )
    ...
    df_all["run_gap"] = (
        df_all.groupby("song_name")["run_index"].diff().fillna(pd.NA).astype("Int64")
    )

    song_stats_1 = (
        df_all.groupby("song_name")
        .agg(
            {
                "running_count": "max",
                "date": ["min", "max"],
    ...
    song_stats_2 = (
        df_all[df_all["inverted_running_count"] <= 10]
        .groupby("song_name")
        .agg({"show_gap": "mean", "run_gap": "mean"})
        .reset_index()
        .rename(
            columns={
                "song_name": "song",
    ...
    song_stats_3 = (
        df_all[df_all["inverted_running_count"] <= 5]
        .groupby("song_name")
        .agg({"show_gap": "mean", "run_gap": "mean"})
        .reset_index()
        .rename(
