...
data_folder = os.path.abspath(os.path.join(SCRIPT_DIR, "../../../3_DataStorage/UM/"))
setlist_path = os.path.join(data_folder, "Collected/setlistdata.csv")
venuedata_path = os.path.join(data_folder, "Collected/venuedata.csv")
...
def evaluate_historical_accuracy(num_shows: int = 50) -> None:
    ...
    for top_n in [25, 50]:
        ...
        for idx, row in target_shows.iterrows():
            ...
            actual_songs = (
                df[df["show_date"] == showdate]["song"].dropna().unique().tolist()
            )
            if not actual_songs:
                log_info(
