    ...
    for top_n in [25, 50]:
        ...
        for idx, row in target_shows.iterrows():
            ...
            actual_songs: List[str] = (
                df[df["showid"] == showid]["song"].dropna().unique().tolist()
            )
            if not actual_songs:
                logger.warning(
...
if __name__ == "__main__":
    ...
    rel_top25 = os.path.relpath(
        os.path.join(prediction_save_dir, "notebook_accuracy_top25.json"),
        start=os.path.join(os.path.dirname(__file__), "../../../"),
    )
    rel_top50 = os.path.relpath(
        os.path.join(prediction_save_dir, "notebook_accuracy_top50.json"),
        start=os.path.join(os.path.dirname(__file__), "../../../"),
    )
