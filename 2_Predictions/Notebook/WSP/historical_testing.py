...
def evaluate_historical_accuracy(num_shows: int = 50) -> None:
    """
    Evaluate prediction accuracy and precision over the last `num_shows` shows for both top 25 and top 50 predictions.
    ...
    """
    df: pd.DataFrame = load_setlist_and_showdata(
        setlist_path, showdata_path, songdata_path
    )
    ...
    for top_n in [25, 50]:
        ...
        for idx, row in target_shows.iterrows():
            ...
            actual_songs: List[str] = (
                df[df["link"] == showid]["song"].dropna().unique().tolist()
            )
            if not actual_songs:
                log_info(
                    f"No actual setlist found for show on {showdate.date()}, skipping."
                )
                continue
            ...
            log_info(
                f"Show {showdate.date()} (top_{top_n}): Recall: {recall:.2%}, Precision: {precision:.2%}"
            )
        results_sorted = sorted(results, key=lambda x: x["showdate"], reverse=True)
        if results_sorted:
            overall_recall = round(
                sum(r["recall"] for r in results_sorted) / len(results_sorted), 5
            )
            overall_precision = round(
                sum(r["precision"] for r in results_sorted) / len(results_sorted), 5
            )
        else:
            overall_recall = None
    ...
    save_json_path = os.path.join(
        data_folder, "Meta/notebook", "historical_accuracy.json"
    )
    os.makedirs(os.path.dirname(save_json_path), exist_ok=True)
    with open(save_json_path, "w") as f:
        json.dump(combined_results, f, indent=2)
    rel_save_json_path = os.path.relpath(
        save_json_path, start=os.path.join(os.path.dirname(__file__), "../../../")
    )
    log_info(
        f"Saved combined accuracy and precision results to Concerts/{rel_save_json_path}"
    )


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Evaluate historical prediction accuracy for both top 25 and top 50 predictions."
    )
    parser.add_argument(
    ...
    rel_json = os.path.relpath(
        os.path.join(prediction_save_dir, "notebook_accuracy.json"),
